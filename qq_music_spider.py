...
word = '告白气球'
res1 = requests.get(
    'https://c.y.qq.com/soso/fcgi-bin/client_search_cp?&t=0&aggr=1&cr=1&catZhida=1&lossless=0&flag_qc=0&p=1&n=20&w='
    + word)
...
for n in range(0, len(mids)):
    res2 = requests.get(
        ...
        'jsonpCallback=MusicJsonCallback&cid=205361747&songmid=' +
...
for m in range(0, x):
    ...
    try:
        # print 'd:/music/'+songnames[m]+' - '+singers[m]+'.m4a'
        urllib.urlretrieve(srcs[m], songnames[m]+' - '+singers[m]+'.m4a')
    except:
        traceback.print_exc()
        x = x - 1
